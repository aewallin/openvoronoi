...
def drawPoint( myscreen, c, pcolor ):
    ca = ovdvtk.Sphere(center=(c.x,c.y,0) , radius=0.002, color=pcolor)
    myscreen.addActor(ca)
...
def rapid_to_next(myscreen, prv_tang, nxt_tang, c1, r1, c2, r2, prv, nxt):
    ...
    #ovdvtk.drawLine(myscreen, prv, nxt, ovdvtk.cyan)
    rad_default = 0.03
    rad = min( rad_default, r1 , r2)
    ...
    prv_tang.normalize()
    nxt_tang.normalize()
    # we start in the direction of prv_tang, and turn until we are in the rapid_tang direction 
...
def rapid_to_new_branch(myscreen, prv_tang, nxt_tang, c1, r1, c2, r2, prv, nxt):
    ...
    # while staying inside c1(r1) and c2(r)
    rad_default = 0.03
    rad1 = min( rad_default, r1 ) # wrong? we get the new-branch r1 here, while we would want the old-branch r1
    rad2 = min( rad_default, r2 )    
    prv_tang.normalize()
    nxt_tang.normalize()
...
if __name__ == "__main__":  
    ...
    while True:
        ...
        if len(mic) >= 2:
            ...
            if not first:
                if new_branch:
                    rapid_to_new_branch(myscreen, out_tangent, in_tangent, previous_center, previous_radius, cen2, r2, previous_out1, in1)
                else:
                ...
                print "on first iter, do nothing."
                first = False
