...
int main(int argc,char *argv[]) {
    ...
    std::cout << "OpenVoronoi version: " << ovd::version() << " build=" << ovd::build_type() << "\n";

    std::vector<int> vertex_ids;
    std::vector<ovd::Point> vertices;
    vertices.push_back( ovd::Point(   0, 0) );   // v0
    vertices.push_back( ovd::Point( -0.1, 0) );  // v1
    vertices.push_back( ovd::Point( 0.2, 0.2) ); // v2
    vertices.push_back( ovd::Point( 0.2, -0.2) );// v3
    //             v2
    //            /
    //    v0-----v1
    //            /
    ...
    vd->insert_line_site( vertex_ids[1], vertex_ids[3]); // will fail?
